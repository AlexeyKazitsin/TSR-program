...
code segment	'code'
	...
	; данные
	ignoredChars 				DB	'йцукенгшщзхъфывапролджёячсмитьбю' ; список игнорируемых символов
	...
	charToCursiveIndex 		DB 'Л' ; символ для замены
	...
	counter	  					DW	0
	printDelay					equ	5 ; таймер вывода сообщения об исполнителе (секунды)
	...
	signatureLine1				DB	179, 'Казицин А.Ю.                                     ', 179
	Line1_length 					equ	$-signatureLine1
	signatureLine2				DB	179, 'ИУ5-42Б                                           ', 179
	Line2_length 					equ	$-signatureLine2
	signatureLine3				DB	179, 'Вариант №8                                       ', 179
	Line3_length 					equ	$-signatureLine3
	helpMsg DB '>tsr.com [/?]', 10, 13
					DB ' [/?] - вывод данной справки', 10, 13
					DB ' Выгрузку осуществляет повторный запуск', 10, 13
					DB ' ===========================================================================', 10, 13
					DB '  F2  - вывод ФИО и группы в верх экрана по таймеру (5 секунд)', 10, 13
					DB '  F3  - вкл/откл курсивного вывода русского символа Л', 10, 13
					DB '  F4  - вкл/откл частичной русификации клавиатуры ({WXIO -> ХЦЧШЩ)', 10, 13
					DB '  F5  - вкл/откл режима ограничения ввода строчных русских букв', 10, 13
					DB ' ===========================================================================', 10, 13
	...
	_checkF1:
		...
		_redF1:
			mov BL, 01001111b ; бирюзовый
			int 10h
			jmp _checkF2

		_greenF1:
			lea BP, f1_txt
			mov BL, 00101111b ; желтый
			int 10h
